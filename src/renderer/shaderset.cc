...
namespace nile {
  ...
  ShaderSet::ShaderSet( u32 id, const std::string &vShaderPath, const std::string &fShaderPath,
                        const std::string &gShaderPath ) noexcept
      : m_programId( new u32( id ) )
      , m_vShaderPath( vShaderPath )
      , m_fShaderPath( fShaderPath )
      , m_gShaderPath( gShaderPath ) {}

  ShaderSet::~ShaderSet() noexcept {
    delete m_programId;
  }
  ...
  std::string ShaderSet::getVshaderPath() const noexcept {
    return m_vShaderPath;
  }

  std::string ShaderSet::getFShaderPath() const noexcept {
    return m_fShaderPath;
  }

  std::string ShaderSet::getGShaderPath() const noexcept {
    return m_gShaderPath;
  }

  void ShaderSet::resetAsset() noexcept {
    glDeleteProgram( *m_programId );
  }

  void ShaderSet::setProgramId( u32 id ) noexcept {
    *m_programId = id;
  }
